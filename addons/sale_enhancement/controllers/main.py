...
#        prod_ids = prod_obj.search(request.cr, request.uid, [('product_tmpl_id', 'in', products_ids), ('public_categ_id.id', '!=', False)], context=request.context)
#        prods = prod_obj.browse(request.cr, request.uid, prod_ids, context=request.context)
#        pprod_ids = []
#        for pp_id in prods:
#            pprod_ids.append(pp_id.public_categ_id.id)
#        pprod_ids = list(set(pprod_ids))
#        print "\n pprod_idsssssssssss", pprod_ids
#        pub_categ = category_obj.browse(request.cr, request.uid, pprod_ids, context=request.context)
#        main_categs = filter(lambda x: not x.parent_id, pub_categ)
#        print "\n ctgssssssssssssssssssssssss", main_categs
        # completed code
        ...
        values = {
            ...
            'styles': styles,
            'category': category,
            'categories': categories, #main_categs,
    ...
    def shop(self, category=None, id=None, page=0, filters='', search='', **post):
        ...
        print "\n categssssssssss intooo shopp", category
        product_obj = request.registry.get('product.template')
        base_domain = request.registry.get('website').ecommerce_get_product_domain()
        domain = list(base_domain)
        if search:
